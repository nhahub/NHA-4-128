...
def chat(req: ChatRequest):
    ...
    # محاولة 1 و 2: الموديل الأساسي (إعادة محاولة تلقائية لو الخطأ tool_use_failed)
    for attempt in range(2):
        try:
            ...
            last_error = e
            if _is_rate_limit(e):
                break  # مفيش داعي نعيد المحاولة على نفس الموديل، روح على الـ fallback على طول
            if attempt == 0 and _is_tool_use_failed(e):
                continue
            break

    # لو الموديل الأساسي وصل لحد الاستخدام اليومي، جرب الموديل الأخف تلقائيًا
