...
# Avoid potential issues with httpx client closure on Windows
...
def get_embeddings() -> HuggingFaceEmbeddings:
    """Get HuggingFace embeddings with retry and offline fallback."""
    import time
    max_retries = 3
    retry_delay = 2
    ...
    for attempt in range(max_retries):
        try:
            print(f"🔄 Loading embeddings model (attempt {attempt + 1}/{max_retries})...")
            ...
            if "client has been closed" in str(e) or "getaddrinfo failed" in str(e):
            ...
            if "No such file" in str(e) and attempt == 0:
                print(f"ℹ️ Downloading embeddings model on first run...")
                time.sleep(1)
                continue
            raise
    
    raise RuntimeError(
        f"Failed to load embeddings model after {max_retries} attempts. "
